...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabase.from("tunes").select(
      `
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!carId) {
      if (!carMake || !carModel)
        return NextResponse.json(
          { error: "carMake and carModel are required" },
          { status: 400 },
        );
      ...
      const safeDrivetrain = validDrivetrains.includes(drivetrain)
        ? drivetrain
        : "RWD";
    ...
    return NextResponse.json(
      { error: message, code: pgErr?.code, details: pgErr?.details },
      { status: 500 },
    );
  }
}
