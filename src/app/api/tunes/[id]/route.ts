  ...
  try {
    ...
    if (user) {
      const [saveRes, upvoteRes] = await Promise.all([
        supabase.from('saves').select('id').eq('user_id', user.id).eq('tune_id', id).maybeSingle(),
        supabase.from('upvotes').select('user_id').eq('user_id', user.id).eq('tune_id', id).maybeSingle(),
      ])
...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    const allowed = ['title', 'description', 'parameters', 'share_code', 'game_version']
