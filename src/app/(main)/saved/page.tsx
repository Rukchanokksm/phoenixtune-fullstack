...
function SavedPageInner() {
  ...
  async function handleUnsave(tuneId: string) {
    setUnsaving(tuneId);
    try {
      const res = await fetch(`/api/saves?tuneId=${tuneId}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Failed to unsave");
      setItems((prev) =>
        prev.filter((item) => normalizeTune(item.tune)?.id !== tuneId),
      );
      ...
      >
        ...
        {!loading && !error && items.length === 0 && (
          ...
          >
            ...
            <h3
              style={{ color: "#f1f5f9", margin: "0 0 8px", fontWeight: 700 }}
            >
              {S.empty}
            </h3>
            <p
              style={{ color: "#475569", margin: "0 0 24px", fontSize: "14px" }}
            >
              {S.emptyDesc}
            </p>
            <Link
