    ...
    try {
      ...
      if (p === 1) {
        const topRes = await fetch(
          `/api/forum/posts?category=general&sortBy=popular&perPage=3${gameParam}`,
        );
      ...
      const res = await fetch(
        `/api/forum/posts?category=general&sortBy=latest&page=${p}&perPage=${PER_PAGE}${gameParam}`,
      );
  ...
  return (
    ...
    >
      ...
      {page === 1 && topPosts.length > 0 && (
        ...
        >
          <div
            style={{ padding: "12px 20px", borderBottom: "1px solid #1a1d24" }}
          >
            <span
              style={{
                color: "#94a3b8",
          ...
          {topPosts.map((p) => (
            <PostRow key={p.id} post={p} highlight />
          ))}
        </div>
      )}
