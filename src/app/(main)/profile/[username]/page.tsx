...
export default async function ProfilePage({ params }: Props) {
    ...
    // Group tunes by game
    const byGame: Record<string, { gameName: string; gameSlug: string; tunes: typeof tuneList }> = {}
    const tuneList = (tunes ?? []).map(t => ({
        ...
        car: (t.car as unknown as { make: string; model: string; pi_class: string } | null),
        gameId: ((t.game as unknown as { id: string; name: string; slug: string } | null))?.id ?? '',
        gameName: ((t.game as unknown as { id: string; name: string; slug: string } | null))?.name ?? '',
        gameSlug: ((t.game as unknown as { id: string; name: string; slug: string } | null))?.slug ?? '',
    }))
    for (const tune of tuneList) {
        if (!tune.gameId) continue
        if (!byGame[tune.gameId])
            byGame[tune.gameId] = { gameName: tune.gameName, gameSlug: tune.gameSlug, tunes: [] }
        byGame[tune.gameId].tunes.push(tune)
    }
    const gameGroups = Object.values(byGame).map(g => ({
        gameName: g.gameName,
        gameSlug: g.gameSlug,
        tunes: g.tunes.map(({ id, title, discipline, upvotes, view_count, created_at, updated_at, car }) =>
            ({ id, title, discipline, upvotes, view_count, created_at, updated_at, car })
        ),
    }))
    ...
    const earned = (profile.titles_earned as string[]).filter(
        (id): id is TitleId =>
            ["newcomer","first_tune","tuner_10","tuner_30","tuner_100","liked_10","liked_50","liked_100"].includes(id),
    )
