...
const S: Record<string, React.CSSProperties> = {
  card:  { background: '#131620', border: '1px solid #1e2330', borderRadius: '16px', padding: '28px' },
  input: { width: '100%', padding: '10px 14px', borderRadius: '10px', background: '#0f1117',
           border: '1px solid #2a2f3f', color: '#f1f5f9', fontSize: '15px',
           outline: 'none', boxSizing: 'border-box' },
  label: { fontSize: '12px', color: '#64748b', fontWeight: 600, display: 'block', marginBottom: '6px' },
...
/* ─── Sub-components ─────────────────────────────────────────── */
function SectionHeader({ emoji, title, dot, enabled, onToggle }: {
  emoji: string; title: string; dot: string; enabled: boolean; onToggle: () => void
}) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
      <span style={{ fontSize: '20px', marginRight: '10px' }}>{emoji}</span>
      <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: '#f1f5f9' }}>{title}</h3>
      <div style={{ width: '6px', height: '6px', borderRadius: '50%',
        background: enabled ? dot : '#374151', margin: '0 10px' }} />
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}
        onClick={onToggle}>
        <span style={{ fontSize: '12px', color: enabled ? '#94a3b8' : '#475569' }}>
          {enabled ? 'ใส่มาด้วย' : 'ไม่ได้ใส่'}
        </span>
        <div style={{ width: '40px', height: '22px', borderRadius: '11px',
          background: enabled ? dot : '#1e2330', border: `1px solid ${enabled ? dot : '#2a2f3f'}`,
          position: 'relative', transition: 'all 0.2s' }}>
          <div style={{ position: 'absolute', top: '3px', left: enabled ? '21px' : '3px',
            width: '16px', height: '16px', borderRadius: '50%',
            background: enabled ? '#fff' : '#475569', transition: 'left 0.2s' }} />
        </div>
      </div>
    </div>
  )
...
function NumInput({ label, value, onChange, onBlur, min, max, step = 0.1, unit, disabled = false }: {
    ...
    <div style={{ flex: 1, minWidth: '140px' }}>
      ...
      <div style={{ position: 'relative' }}>
        <input type="number" step={step} min={min} max={max}
          ...
          onBlur={onBlur}
          style={{ ...S.input, paddingRight: unit ? '46px' : '14px', opacity: disabled ? 0.35 : 1 }}
        />
        {unit && (
          <span style={{ position: 'absolute', right: '12px', top: '50%',
            transform: 'translateY(-50%)', fontSize: '11px', color: '#475569' }}>
            {unit}
          </span>
        )}
      </div>
...
function SubToggle({ label, enabled, onToggle }: { label: string; enabled: boolean; onToggle: () => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px',
      cursor: 'pointer', fontSize: '12px', color: '#64748b', marginLeft: 'auto' }}
      onClick={onToggle}>
      <div style={{ width: '32px', height: '18px', borderRadius: '9px',
        background: enabled ? '#6366f1' : '#1e2330', border: `1px solid ${enabled ? '#6366f1' : '#2a2f3f'}`,
        position: 'relative', transition: 'all 0.2s' }}>
        <div style={{ position: 'absolute', top: '2px', left: enabled ? '16px' : '2px',
...
function Chip({ label, active, onClick, color }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? color : '#1a1d24', color: active ? '#0d0f14' : color,
...
export default function ShareTunePage() {
  ...
  const setGear = (i: number, v: string) => { const g = [...gears]; g[i] = v; setGears(g) }

  const [alignOn, setAlignOn]   = useState(false)
  const [camberOn, setCamberOn] = useState(true)
  const [cF, setCF] = useState(''); const [cR, setCR] = useState('')
  const [toeOn, setToeOn]       = useState(true)
  const [toF, setToF] = useState(''); const [toR, setToR] = useState('')
  const [castOn, setCastOn]     = useState(true)
  const [cast, setCast]         = useState('')

  const [arbOn, setArbOn] = useState(false)
  const [aF, setAF] = useState(''); const [aR, setAR] = useState('')

  const [springOn, setSpringOn] = useState(false)
  const [spF, setSpF] = useState(''); const [spR, setSpR] = useState('')
  const [rhF, setRhF] = useState(''); const [rhR, setRhR] = useState('')

  const [dampOn, setDampOn] = useState(false)
  const [rbF, setRbF] = useState(''); const [rbR, setRbR] = useState('')
  const [buF, setBuF] = useState(''); const [buR, setBuR] = useState('')

  const [aeroOn, setAeroOn] = useState(false)
  const [arF, setArF] = useState(''); const [arR, setArR] = useState('')
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px',
        ...
        <div style={S.card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: '#f1f5f9' }}>เลือกรถ</h3>
          </div>
          <div style={S.row}>
          ...
          <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #1e2330' }}>
            <label style={{ ...S.label, marginBottom: '12px' }}>Class / PI</label>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {CAR_CLASSES.map(cls => {
                const active = carClass === cls.id
                return (
                  <button key={cls.id} onClick={() => setCarClass(active ? '' : cls.id)} style={{
                    padding: '8px 18px', borderRadius: '8px', fontSize: '14px', fontWeight: 800,
          ...
          <>
            ...
            <div style={S.card}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
                ...
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: '#f1f5f9' }}>Tires</h3>
                <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#60a5fa', margin: '0 10px' }} />
                <span style={{ marginLeft: 'auto', fontSize: '12px', color: '#475569' }}>หน่วย: bar (1.0 – 3.8)</span>
              </div>
              <div style={S.row}>
                <NumInput label="Front" value={tF} unit="bar" min={1.0} max={3.8} step={0.1}
                  onChange={setTF} onBlur={() => setTF(clamp(tF, 1.0, 3.8, 1))} />
                <NumInput label="Rear"  value={tR} unit="bar" min={1.0} max={3.8} step={0.1}
                  onChange={setTR} onBlur={() => setTR(clamp(tR, 1.0, 3.8, 1))} />
              </div>
            </div>

            {/* Gearing */}
            <div style={S.card}>
              <SectionHeader emoji="⚙️" title="Gearing" dot="#f59e0b"
                enabled={gearOn} onToggle={() => setGearOn(p => !p)} />
              {gearOn && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <Chip label="Final Drive Only" active={gearType === 'final'} color="#f59e0b" onClick={() => setGearType('final')} />
                    <Chip label="Full Gearing"     active={gearType === 'full'}  color="#f59e0b" onClick={() => setGearType('full')} />
                  </div>
                  <div style={{ maxWidth: '200px' }}>
                    <NumInput label="Final Drive  (0.00 – 6.10)" value={gearFinal} min={0} max={6.10} step={0.01}
                      onChange={setGearFinal} onBlur={() => setGearFinal(clamp(gearFinal, 0, 6.10, 2))} />
                  </div>
                  {gearType === 'full' && (
                    <>
                      <div>
                        <label style={S.label}>จำนวนเกียร์</label>
                        <select value={gearCount} onChange={e => setGearCount(parseInt(e.target.value))}
                          style={{ ...S.input, width: 'auto', minWidth: '130px', cursor: 'pointer' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                        {Array.from({ length: gearCount }, (_, i) => i).map(i => (
                          <div key={i} style={{ flex: '0 0 calc(25% - 9px)', minWidth: '100px' }}>
                            <NumInput label={`Gear ${i + 1}  (0.00–6.00)`} value={gears[i]}
                              min={0} max={6.0} step={0.01}
                              onChange={v => setGear(i, v)}
                              onBlur={() => setGear(i, clamp(gears[i], 0, 6.0, 2))} />
                          </div>
                        ))}
                      </div>
                    </>
            ...
            {/* Alignment */}
            <div style={S.card}>
              <SectionHeader emoji="📐" title="Alignment" dot="#a78bfa"
                enabled={alignOn} onToggle={() => setAlignOn(p => !p)} />
              {alignOn && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
                      <span style={S.sub}>Camber</span><span style={S.hint}>(°) −5.0 ~ 5.0</span>
                      <SubToggle label="ใส่มาด้วย" enabled={camberOn} onToggle={() => setCamberOn(p => !p)} />
                    </div>
                    <div style={S.row}>
                      <NumInput label="Front" value={cF} unit="°" min={-5} max={5} step={0.1}
                        disabled={!camberOn} onChange={setCF} onBlur={() => setCF(clamp(cF, -5, 5, 1))} />
                      <NumInput label="Rear"  value={cR} unit="°" min={-5} max={5} step={0.1}
                        disabled={!camberOn} onChange={setCR} onBlur={() => setCR(clamp(cR, -5, 5, 1))} />
                    </div>
                  </div>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
                      <span style={S.sub}>Toe</span><span style={S.hint}>(°) −5.0 ~ 5.0</span>
                      <SubToggle label="ใส่มาด้วย" enabled={toeOn} onToggle={() => setToeOn(p => !p)} />
                    </div>
                    <div style={S.row}>
                      <NumInput label="Front" value={toF} unit="°" min={-5} max={5} step={0.1}
                        disabled={!toeOn} onChange={setToF} onBlur={() => setToF(clamp(toF, -5, 5, 1))} />
                      <NumInput label="Rear"  value={toR} unit="°" min={-5} max={5} step={0.1}
                        disabled={!toeOn} onChange={setToR} onBlur={() => setToR(clamp(toR, -5, 5, 1))} />
                    </div>
                  </div>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
                      <span style={S.sub}>Front Caster</span><span style={S.hint}>(°) 1.0 ~ 7.0</span>
                      <SubToggle label="ใส่มาด้วย" enabled={castOn} onToggle={() => setCastOn(p => !p)} />
                    </div>
                    <div style={{ maxWidth: '200px' }}>
                      <NumInput value={cast} unit="°" min={1} max={7} step={0.1}
                        disabled={!castOn} onChange={setCast} onBlur={() => setCast(clamp(cast, 1, 7, 1))} />
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Anti-Roll Bars */}
            <div style={S.card}>
              <SectionHeader emoji="🔧" title="Anti-Roll Bars" dot="#38bdf8"
                enabled={arbOn} onToggle={() => setArbOn(p => !p)} />
              {arbOn && (
                <div style={S.row}>
                  <NumInput label="Front  (1 – 65)" value={aF} min={1} max={65} step={0.01}
                    onChange={setAF} onBlur={() => setAF(clamp(aF, 1, 65, 2))} />
                  <NumInput label="Rear  (1 – 65)"  value={aR} min={1} max={65} step={0.01}
                    onChange={setAR} onBlur={() => setAR(clamp(aR, 1, 65, 2))} />
                </div>
              )}
            </div>
            ...
            {/* Springs & Ride Height */}
            <div style={S.card}>
              <SectionHeader emoji="🌀" title="Springs & Ride Height" dot="#fb923c"
                enabled={springOn} onToggle={() => setSpringOn(p => !p)} />
              {springOn && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div>
                    <div style={{ marginBottom: '12px' }}>
                      <span style={S.sub}>Spring Rate</span><span style={S.hint}>N/MM  (0 – 9999.9)</span>
                    </div>
                    <div style={S.row}>
                      <NumInput label="Front" value={spF} unit="N/MM" min={0} max={9999.9} step={0.1}
                        onChange={setSpF} onBlur={() => setSpF(clamp(spF, 0, 9999.9, 1))} />
                      <NumInput label="Rear"  value={spR} unit="N/MM" min={0} max={9999.9} step={0.1}
                        onChange={setSpR} onBlur={() => setSpR(clamp(spR, 0, 9999.9, 1))} />
                    </div>
                  </div>
                  <div>
                    <div style={{ marginBottom: '12px' }}>
                      <span style={S.sub}>Ride Height</span><span style={S.hint}>CM  (0 – 99.9)</span>
                    </div>
                    <div style={S.row}>
                      <NumInput label="Front" value={rhF} unit="CM" min={0} max={99.9} step={0.1}
                        onChange={setRhF} onBlur={() => setRhF(clamp(rhF, 0, 99.9, 1))} />
                      <NumInput label="Rear"  value={rhR} unit="CM" min={0} max={99.9} step={0.1}
                        onChange={setRhR} onBlur={() => setRhR(clamp(rhR, 0, 99.9, 1))} />
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Damping */}
            <div style={S.card}>
              <SectionHeader emoji="💧" title="Damping" dot="#c084fc"
                enabled={dampOn} onToggle={() => setDampOn(p => !p)} />
              {dampOn && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  {([
                    ['Rebound Stiffness', rbF, setRbF, rbR, setRbR],
                    ['Bump Stiffness',    buF, setBuF, buR, setBuR],
                  ] as const).map(([title, fv, fs, rv, rs]) => (
                    <div key={title}>
                      <div style={{ marginBottom: '12px' }}>
                        <span style={S.sub}>{title}</span><span style={S.hint}>1.0 – 20.0</span>
                      </div>
                      <div style={S.row}>
                        <NumInput label="Front" value={fv} min={1} max={20} step={0.1}
                          onChange={v => fs(v)} onBlur={() => fs(clamp(fv, 1, 20, 1))} />
                        <NumInput label="Rear"  value={rv} min={1} max={20} step={0.1}
                          onChange={v => rs(v)} onBlur={() => rs(clamp(rv, 1, 20, 1))} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Brake */}
            <div style={S.card}>
              <SectionHeader emoji="🛑" title="Brake" dot="#f87171"
                enabled={brakeOn} onToggle={() => setBrakeOn(p => !p)} />
              {brakeOn && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div>
                    <div style={{ marginBottom: '12px' }}>
                    ...
                    <div style={{ maxWidth: '300px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ fontSize: '11px', color: '#60a5fa', fontWeight: 700 }}>F</span>
                        <input type="range" min={0} max={100} value={bBal}
                          onChange={e => setBBal(e.target.value)}
                          style={{ flex: 1, accentColor: '#f87171' }} />
                        <span style={{ fontSize: '11px', color: '#f87171', fontWeight: 700 }}>R</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <input type="number" min={0} max={100} step={1} value={bBal}
                          onChange={e => setBBal(e.target.value)}
                          onBlur={() => setBBal(clamp(bBal, 0, 100, 0))}
                        ...
                        <span style={{ fontSize: '12px', color: '#374151', marginLeft: '4px' }}>
                          {parseInt(bBal) <= 50 ? '← Front biased' : 'Rear biased →'}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div>
                    <div style={{ marginBottom: '12px' }}>
                      <span style={S.sub}>Braking Pressure</span><span style={S.hint}>%  (0 – 200)</span>
                    </div>
                    <div style={{ maxWidth: '160px' }}>
                      <NumInput value={bPre} unit="%" min={0} max={200} step={1}
                        onChange={setBPre} onBlur={() => setBPre(clamp(bPre, 0, 200, 0))} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {diffOn && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      {(['AWD', 'RWD', 'FWD'] as const).map(t => (
                        <Chip key={t} label={t} active={diffType === t} color="#fbbf24"
            ...
            <div style={S.card}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                <span style={{ fontSize: '20px' }}>🔩</span>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: '#f1f5f9' }}>Upgrades</h3>
                <span style={{ fontSize: '12px', color: '#475569', marginLeft: '4px' }}>optional — บอกว่าใส่ของแต่งอะไรมาบ้าง</span>
              </div>
              <p style={{ margin: '0 0 16px', fontSize: '13px', color: '#475569' }}>
                ระบุ upgrade ที่ใช้ใน tune นี้ (รายละเอียดเร็วๆ นี้)
              </p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {['Engine', 'Platform & Handling', 'Drivetrain', 'Tires & Wheels', 'Aero & Appearance', 'Conversion'].map(cat => (
                  <div key={cat} style={{ padding: '10px 16px', borderRadius: '10px',
                    ...
                    {cat}
                    <span style={{ fontSize: '10px', color: '#2a2f3f',
                      background: '#1a1d24', padding: '2px 7px', borderRadius: '6px' }}>
                      เร็วๆ นี้
                    </span>
                  </div>
                ))}
