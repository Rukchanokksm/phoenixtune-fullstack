...
export function EditProfileForm({ userId, initial, username }: Props) {
  ...
  return (
    <div style={{ background: '#111318', border: '1px solid #1e2130', borderRadius: '16px', marginBottom: '24px', overflow: 'hidden' }}>
      {/* Toggle header */}
      <button
        onClick={() => setOpen(o => !o)}
        style={{
      ...
      >
        <span style={{ fontWeight: 700, fontSize: '15px' }}>✏️ แก้ไขข้อมูลส่วนตัว</span>
        <span style={{ color: '#475569', fontSize: '18px', transition: 'transform 0.2s', transform: open ? 'rotate(180deg)' : 'none' }}>▾</span>
      </button>

      {open && (
        <form onSubmit={handleSave} style={{ padding: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ height: '1px', background: '#1e2130', margin: '0 0 4px' }} />
          ...
          <button type="submit" disabled={loading}
            style={{ padding: '11px', borderRadius: '9px', border: 'none', fontWeight: 800, fontSize: '14px', cursor: loading ? 'not-allowed' : 'pointer',
