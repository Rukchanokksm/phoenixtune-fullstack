...
export function TitlesEarnedSection({ titleIds }: { titleIds: TitleId[] }) {
    ...
    return (
        <div style={{ background: '#111318', border: '1px solid #1e2130', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
            <h2 style={{ margin: '0 0 16px', color: '#e2e8f0', fontWeight: 700, fontSize: '16px' }}>
                🏅 {tt.sectionHeader}
            </h2>
            {titleIds.length === 0 ? (
                <p style={{ color: '#475569', fontSize: '14px', margin: 0 }}>{tt.noTitles}</p>
            ) : (
                    ...
                    {titleIds.map((id) => {
                        ...
                        return (
                            <span key={id} style={{
                                ...
                                color: meta.color, fontSize: '13px', fontWeight: 600,
                            }}>
